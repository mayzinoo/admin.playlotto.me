			...
			<table class="table">
				<thead>
						<tr>
						  ...
						  <th>Value in Lotto</th>
						  <th>Paid Amount</th>
						  <th>Current Balance</th>
						  <th>Request Info</th>
						 <!-- <th>Depositor's Name</th>-->
						  <th style="vertical-align:top">Requested Date</th>
				...
				<tbody>
					<?php
					$i=1;
				foreach($depositdata->result() as $row): ?>
						<tr>
						  <td><?php echo $i; ?></td>
	...
	<?php }else{ ?>
<td style="line-height:1rem;width:25%"><button onclick="confirmdeposit('<?=$row->id?>','<?=$row->user_id?>','<?=$row->amt?>','<?=$row->bal?>','<?=$row->currency_type?>')" style="padding: 2px 7px;
    border-radius: 4px;
    color: #fff;
    background-color: #5cb85c;
    border-color: #4cae4c;" id="txtchg<?=$row->id?>">Confirm
<button onclick="canceldeposit('<?=$row->id?>','<?=$row->user_id?>','<?=$row->amt?>','<?=$row->bal?>','<?=$row->currency_type?>')" style="margin-left:2px;padding: 2px 7px;
    border-radius: 4px;
    color: #fff;
    background-color: #ff0000;
    border-color:#ff0000;" class="btn-side" id="txtcancel<?=$row->id?>">Cancel</button>
<a href="/Admin/editlotto/<?php echo $row->user_id; ?>/<?php echo $row->id; ?>" class="update-btn btn btn-success" id="txtupdate<?=$row->id?>">Update</a>
</td>
	<?php	} ?>
					</td>
						</tr>
